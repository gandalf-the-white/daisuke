...
resource "local_file" "inventory" {
  ...
  filename        = "./ansible/inventory-postgres.yaml"
...
resource "local_file" "playbook" {
  ...
  filename        = "./ansible/playbook-postgres.yaml"
  file_permission = "0644"
}

resource "null_resource" "play_ansible" {
  provisioner "local-exec" {
    command = "ansible-playbook -i ansible/inventory-postgres.yaml ansible/playbook-postgres.yaml"
  }
  depends_on = [
